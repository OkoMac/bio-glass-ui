    ...
    <div className="min-h-screen bg-obsidian pb-40">
      ...
      <div className="relative h-[240px] md:h-[320px] overflow-hidden">
        ...
        <div className="absolute top-8 md:top-12 left-4 right-4 flex justify-between">
          <motion.button whileTap={{ scale: 0.9 }} onClick={() => navigate(-1)} aria-label="Go back" className="glass-2 rounded-full w-10 h-10 flex items-center justify-center">
          ...
          <div className="flex gap-2">
            ...
            <motion.button whileTap={{ scale: 0.9 }}
              ...
              <svg viewBox="0 0 24 24" className="w-5 h-5 fill-[#25D366]" aria-hidden="true"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347z"/><path d="M12 0C5.373 0 0 5.373 0 12c0 2.625.846 5.059 2.284 7.034L.789 23.492a.5.5 0 00.611.611l4.458-1.495A11.94 11.94 0 0012 24c6.627 0 12-5.373 12-12S18.627 0 12 0zm0 22c-2.34 0-4.508-.657-6.363-1.795l-.444-.267-3.072 1.03 1.03-3.072-.267-.444A9.96 9.96 0 012 12C2 6.477 6.477 2 12 2s10 4.477 10 10-4.477 10-10 10z"/></svg>
            </motion.button>
            <motion.button whileTap={{ scale: 0.9 }} onClick={shareProvider} aria-label="Share this provider" className="glass-2 rounded-full w-10 h-10 flex items-center justify-center">
              {copied ? <Check className="w-5 h-5 text-teal" aria-hidden="true" /> : <Share2 className="w-5 h-5 text-foreground" aria-hidden="true" />}
            </motion.button>
          </div>
        </div>
          ...
          <section>
            ...
            <GlassCard className="p-4 mt-3 space-y-3">
              ...
              <p className="text-xs text-muted-foreground">
                {provider.contact?.website
                  ? `${provider.name.split(/[—–-]/)[0].trim()} hasn't listed individual services on BION yet — view their full menu on their site or message them directly.`
                  : `${provider.name.split(/[—–-]/)[0].trim()} hasn't listed individual services on BION yet. Message them directly for a full menu and pricing.`}
              </p>
              {/* Primary CTA — view their menu on the provider's own site
                  when we have a URL. BION is the discovery surface; the
                  provider's site is the source of truth for menus. */}
              {provider.contact?.website && (() => {
                const url = provider.contact.website.startsWith("http")
                  ? provider.contact.website
                ...
                return (
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="w-full rounded-pill py-2.5 text-xs font-semibold gradient-indigo text-primary-foreground flex items-center justify-center gap-2 shadow-cta"
                  >
                    <Globe className="w-4 h-4" />
                    View full menu on {domain}
                  </a>
                );
              })()}
              <button
