          ...
          <div className="flex gap-2 overflow-x-auto pb-2 hide-scrollbar">
            {clients.map(c => (
              <button
               ...
               title="setSelectedClient(c.id)} className= `} > )}" aria-label="setSelectedClient(c.id)} className= `} > )}">
                {c.avatar_url ? (
            ...
            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }}>
              <GlassCard className="p-5 space-y-4">
                ...
                <div>
                          ...
                          <div className="flex gap-1.5 flex-wrap flex-1">
                            {ITEM_TYPES.map(t => (
                              <button key={t.value} onClick={() => updateItem(idx, "type", t.value)}
                                ...
                                }`} title="updateItem(idx, 'type', t.value)} className= `}>" aria-label="updateItem(idx, 'type', t.value)} className= `}>">
                                {t.label}
                              </button>
                            ))}
                          </div>
                          {items.length > 1 && (
                            <button onClick={() => removeItem(idx)} className="w-6 h-6 rounded-full glass-2 flex items-center justify-center" title="removeItem(idx)} className='w-6 h-6 rounded-full glass-2 flex items-center ju…" aria-label="removeItem(idx)} className='w-6 h-6 rounded-full glass-2 flex items-center ju…">
                  ...
                  <button onClick={addItem} className="mt-2 flex items-center gap-1.5 text-xs text-indigo font-medium" title="Add Item" aria-label="Add Item">
                          ...
                          <div className="flex gap-2 pt-2">
                            {plan.status === "active" && (
                              <>
                                <button onClick={() => updatePlanStatus(plan.id, "paused")}
                                  className="flex-1 py-2 glass-1 rounded-pill text-xs font-medium text-amber" title="updatePlanStatus(plan.id, 'paused')} className='flex-1 py-2 glass-1 rounded-p…" aria-label="updatePlanStatus(plan.id, 'paused')} className='flex-1 py-2 glass-1 rounded-p…">
                                  Pause
                                </button>
                                <button onClick={() => updatePlanStatus(plan.id, "completed")}
                                  className="flex-1 py-2 glass-1 rounded-pill text-xs font-medium text-teal" title="updatePlanStatus(plan.id, 'completed')} className='flex-1 py-2 glass-1 rounde…" aria-label="updatePlanStatus(plan.id, 'completed')} className='flex-1 py-2 glass-1 rounde…">
                                  Mark Complete
                                </button>
                              </>
                            )}
                            {plan.status === "paused" && (
                              <button onClick={() => updatePlanStatus(plan.id, "active")}
                                className="flex-1 py-2 glass-1 rounded-pill text-xs font-medium text-indigo" title="updatePlanStatus(plan.id, 'active')} className='flex-1 py-2 glass-1 rounded-p…" aria-label="updatePlanStatus(plan.id, 'active')} className='flex-1 py-2 glass-1 rounded-p…">
                                Resume
                              </button>
                            )}
                          </div>
