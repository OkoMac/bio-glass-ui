...
export default function NotificationBell() {
  ...
  return (
    <motion.button
      ...
      className="fixed top-4 right-4 z-[45] w-10 h-10 rounded-full glass-2 flex items-center justify-center shadow-card"
      aria-label={`Notifications${totalCount > 0 ? ` (${totalCount})` : ""}`}
    >
      <Bell className="w-4.5 h-4.5 text-foreground" strokeWidth={1.5} />
      {totalCount > 0 && (
